...
def u_dict(u):
    ...
    return {
        ...
        'badge_type':u.badge_type,'bio':u.bio or '',
        'avatar_url':f'/avatar/{u.id}' if u.avatar_url else '',
...
def avatar(uid):
    ...
    if not u or not u.avatar_url or not u.avatar_url.startswith('data:'):
        return '', 404
...
def invite_to_chat(cid):
    ...
    if not u:
        return jsonify({'error':'Не найден'}),404
    if ChatMember.query.filter_by(chat_id=cid,user_id=u.id).first():
        return jsonify({'error':'Уже в чате'}),400
    db.session.add(ChatMember(chat_id=cid,user_id=u.id))
    db.session.commit()
    return jsonify({'message':'OK'})
