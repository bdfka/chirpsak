...
def u_dict(u):
    if not u:
        return {'username':'?','verified':False,'badge_type':'blue','bio':'','avatar_url':'','online':False,'last_seen':''}
    return {
        'id':u.id,'username':u.username,'verified':u.verified,
        'badge_type':u.badge_type,'bio':u.bio or '','avatar_url':u.avatar_url or '',
        'online':u.online,'last_seen':str(u.last_seen)
    }
...
def posts():
    ...
    return jsonify([{
        ...
        'timestamp':str(p.timestamp),'author':u_dict(User.query.get(p.user_id))
    } for p in ps])
...
def like(pid):
    Post.query.get_or_404(pid).likes += 1
    db.session.commit()
    return jsonify({'ok':True})
...
def profile(username):
    ...
    return jsonify({
        'user':u_dict(u),
        'posts':[{'id':p.id,'content':p.content,'likes':p.likes,'timestamp':str(p.timestamp)} for p in ps],
        'friends':fr
    })
...
def verify():
    ...
    if not u:
        return jsonify({'error':'Не найден'}),404
    u.verified = True
    u.badge_type = d.get('badge_type','blue')
    db.session.commit()
...
def ch_pass():
    d = request.get_json()
    u = User.query.get(d['user_id'])
    if not u or not bcrypt.check_password_hash(u.password,d.get('old_password','')):
        return jsonify({'error':'Неверный пароль'}),400
    if d.get('email') != u.email:
        return jsonify({'error':'Email не совпадает'}),400
    u.password = bcrypt.generate_password_hash(d['new_password']).decode('utf-8')
    db.session.commit()
    return jsonify({'message':'OK'})
...
def upload_avatar():
    ...
    u.avatar_url = f"/static/avatars/{fn}"
    db.session.commit()
...
def send_fr():
    ...
    if not to:
        return jsonify({'error':'Не найден'}),404
    if int(d['from_id']) == to.id:
        return jsonify({'error':'Нельзя добавить себя'}),400
    existing = FriendRequest.query.filter_by(from_id=int(d['from_id']),to_id=to.id,status='pending').first()
    if existing:
        return jsonify({'error':'Уже отправлена'}),400
    are_friends = Friendship.query.filter(
        ((Friendship.user1_id==int(d['from_id']))&(Friendship.user2_id==to.id)) |
        ((Friendship.user1_id==to.id)&(Friendship.user2_id==int(d['from_id'])))
    ).first()
    if are_friends:
        return jsonify({'error':'Вы уже друзья'}),400
    db.session.add(FriendRequest(from_id=int(d['from_id']),to_id=to.id))
    db.session.commit()
    return jsonify({'message':'OK'})
...
def fr_list(uid):
    ...
    return jsonify({
        'incoming':[{'id':r.id,'from':u_dict(User.query.get(r.from_id))} for r in inc],
        'outgoing':[{'id':r.id,'to':u_dict(User.query.get(r.to_id))} for r in out]
    })

@app.route('/friend-request/<int:rid>/accept', methods=['POST'])
def accept_fr(rid):
    r = FriendRequest.query.get_or_404(rid)
    r.status = 'accepted'
    db.session.add(Friendship(user1_id=r.from_id,user2_id=r.to_id))
    db.session.commit()
    return jsonify({'message':'OK'})

@app.route('/friend-request/<int:rid>/reject', methods=['POST'])
def reject_fr(rid):
    FriendRequest.query.get_or_404(rid).status = 'rejected'
    db.session.commit()
    return jsonify({'message':'OK'})
...
def mutual(uid1, username):
    ...
    f1 = set()
    for f in Friendship.query.filter((Friendship.user1_id==uid1)|(Friendship.user2_id==uid1)).all():
        f1.add(f.user2_id if f.user1_id==uid1 else f.user1_id)
    f2 = set()
    for f in Friendship.query.filter((Friendship.user1_id==u2.id)|(Friendship.user2_id==u2.id)).all():
        f2.add(f.user2_id if f.user1_id==u2.id else f.user1_id)
...
def create_chat():
    ...
    if d.get('other_user_id'):
        db.session.add(ChatMember(chat_id=c.id,user_id=int(d['other_user_id'])))
    db.session.commit()
...
def msgs(cid):
    ...
    return jsonify([{
        'id':m.id,'content':m.content,'timestamp':str(m.timestamp),
        'sender':u_dict(User.query.get(m.sender_id))
    } for m in ms])
